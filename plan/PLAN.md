...
| 5d | WebApp viewer + relay forwarding | ⏳ | Vendor `r2-wasm` from r2-core into `wasm-viewer/`; build static bundle for **GitHub Pages**. One WebApp binary, two modes: **onsite** (browser links to the onsite host over the hotspot) and **remote** (browser links via the relay). Browser detects mode automatically by which transport succeeds. Onsite mode = full operator control (Start/Mark/Stop); **remote mode = read-only by default — must not upset a running experiment** (enforced by TG cert role + UI). Each browser is its own TG member (keypair in IndexedDB, TG-signed cert via enrolment code from the onsite dashboard, per `r2-trust` §2). UX layer stays plain JS + Chart.js; WASM handles only protocol + crypto. Same deployment shape as r2-notekeeper. Initially uses public r2-relay; migrates to own relay+archive (5e) by config swap. |
