...
| 5d-enrol | QR / link enrolment UX | ⏳ | Onsite dashboard exposes an "Enrol device" button → generates a one-time join token (single-use, ≤5 min expiry, signed by KeyHolder so it can't be forged) → renders a QR code on the dashboard screen AND a shareable link `https://<github-pages>/r2-rocker/?join=<token>`. The WebApp on first load reads `?join=` from the URL, generates its keypair in IndexedDB, sends `public_key + token` to the onsite KeyHolder via the relay, gets back a TG-signed device cert, persists, becomes a TG member. Scan-and-go onboarding for laptops, phones, tablets — same flow. Fallback: manual paste of the join link on a "not enrolled" landing page. |
