...
| 5d-ensemble | Author `r2-rocker.ensemble.yaml` + refactor to sentants | ⏳ | Per the 2026-05-07 conformance audit: define the canonical ensemble YAML (R2-DEF §7), refactor firmware to run a sensor sentant on `r2-engine` with plugins for SD-storage / battery / WiFi / accelerometer / OTA, refactor onsite dashboard's Rust process into bridge + calibration + archive sentants. Same shape as `r2-notekeeper/ensemble/ensemble.yaml`. **Not a Phase 5a blocker** — the wire-level work ships first. |
