...
| `r2.dash.ota.progress` | controller → viewer | OTA push lifecycle (uploading / applied / rejected / error). v0.1 of this sentant subscribes for `event_count` bookkeeping only. Per SPEC-R2-ROCKER-WIRE row 23. |
| `r2.dash.reset.progress` | controller → viewer | Sensor-reset lifecycle (requested / applied / error). v0.1 bookkeeping-only. Per SPEC-R2-ROCKER-WIRE row 24. |
...
SPEC-R2-ROCKER-WIRE §2 rows 22-28); promoting `peek_state()` to
